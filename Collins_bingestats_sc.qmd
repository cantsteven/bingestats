...
Creating a new tables for summary data:

```{r}
# creating a new table showing every bodies data 
binge_table <- binge_stats_filtered |> 
  select(study, binge_date, sex, unique_id)

#Mean intox

behav_intox_average_rename <- behav_intox_average |> 
rename(mean_intoxication = mean)

#Peak intox
behav_intox_peak_rename <- behav_intox_peak |> 
  rename(peak_intoxication = mean)

#Dose 

dose_day_mean_rename <- dose_day_mean |> 
  rename(mean_dose = mean)

#joining all mean binge data into one main table 
binge_table2 <- binge_table |> 
  full_join(behav_intox_average_rename) |> 
  full_join(behav_intox_peak_rename) |> 
  full_join(dose_day_mean_rename) |> 
  full_join(bec) |> 
  full_join(withdrawal_grouped_average) |> 
  full_join(withdrawal_grouped_peak)

#full summary of all binges used in the paper 

binge_table3 <- binge_table2 |>
  group_by(study, sex) |> 
  summarise(Binge = print(study), date = print(binge_date),n = n(), 
            mean_intox = mean(mean_intoxication), sem_mean_intox = sd(mean_intoxication) / sqrt(length(mean_intoxication)), 
            peak_intox = mean(peak_intoxication), sem_peak_intox = sd(peak_intoxication) / sqrt(length(peak_intoxication)), 
            mean_dose_by_day = mean(mean_dose), sem_mean_dose_by_day = sd(mean_dose) / sqrt(length(mean_dose)),
            bec_mean = mean(mean_bec), sem_bec_mean = sd(mean_bec) / sqrt(length(mean_bec)),
            mean_with = mean(mean_with_score), sem_mean_with = sd(mean_with_score) / sqrt(length(mean_with_score)), 
            peak_with = mean(max_with_score), sem_peak_with = sd(max_with_score) / sqrt(length(max_with_score))) |>
  distinct() |> 
  arrange(study, sex, date)
```
...
# creating PCA table 
binge_pca_data_table <- binge_table2 |> 
  select("sex", "mean_intoxication", "peak_intoxication", "mean_dose", "mean_bec", "mean_with_score", "max_with_score") |>
...
pca_proportion <- ggplot(my_pca_table, aes(x = PC, y = Variance)) +
...
#cumulative of variance graph 
pca_cumulative <- ggplot(my_pca_table, aes(x = PC, y = Cumulative)) +
...
binge_PCA_scores$sex <- binge_pca_data_table$sex
...
pca_plot <- ggplot() +
  ...
  coord_equal() + 
  theme(legend.position = "none") +
  scale_x_continuous(limits = c(-6.0, 6.1), 
                     expand = c(0,0), 
                     breaks = seq(-6,6.1)) +
...
ggsave("binge_pca_plot.png", plot = pca_plot2, width = 7, height = 6, dpi = 600, units = "in")\
\
\
\
PCA Density Plots
...
# Plot
PC1_density <- ggplot(binge_PCA_scores, aes(x = Comp.1, fill = sex)) +
  ...
  scale_x_continuous(limits = c(-6.0, 6.3), 
                     expand = c(0,0), 
                     breaks = seq(-6.0,6.3, by = 2.0)) +
  scale_y_continuous(
    limits = c(0.0, 1.0), 
    expand = c(0, 0), 
    breaks = seq(0.0, 1.0)) + labs(y = NULL)

PC1_density


PC2_density <- ggplot(binge_PCA_scores, aes(x = Comp.2, fill = sex)) +
  geom_density(alpha = 0.5, color = "black") +
  labs(title = "",
       x = "PC2 Score") +
  theme_prism() +
...
PC3_density <- ggplot(binge_PCA_scores, aes(x = Comp.3, fill = sex)) +
  geom_density(alpha = 0.5, color = "black") +
  labs(title = "",
       x = "PC3 Score") +
  theme_prism() +
...
PC4_density <- ggplot(binge_PCA_scores, aes(x = Comp.4, fill = sex)) +
  geom_density(alpha = 0.5, color = "black") +
  labs(title = "",
       x = "PC4 Score") +
  theme_prism() +
...
PC5_density <- ggplot(binge_PCA_scores, aes(x = Comp.5, fill = sex)) +
  geom_density(alpha = 0.5, color = "black") +
  labs(title = "",
       x = "PC5 Score") +
  theme_prism() +
...
PC6_density <- ggplot(binge_PCA_scores, aes(x = Comp.6, fill = sex)) +
  geom_density(alpha = 0.5, color = "black") +
  labs(title = "",
       x = "PC6 Score") +
  theme_prism() +
  ...
  scale_y_continuous(
    limits = c(0.0, 2.0), 
    expand = c(0, 0), 
    breaks = seq(0.0, 2.0, by = 1)) + labs(y = NULL)
...
ggsave("PC5_density_plot.png", plot = PC5_density, width = 4, height = 4, dpi = 600, units = "in", limitsize = FALSE)\
